...
class PolydownController:
    ...
    def _generate_texture_tasks(self, asset_id, data, root_folder, target_sizes, overwrite, noimgs, tone, texture_format, maps, asset_type="textures"):
        ...
        available_maps = [k for k in data.keys() if k not in ignored_keys]
        ...
        for map_name in available_maps:
            ...
            # iterate sizes
            for size, formats in map_data.items():
                if target_sizes and size not in target_sizes:
                    continue
                
                # Check formats
                # If texture_format is set, look for it. Else, default order preference? Or all?
                # The user request implies they want specific formats.
                # If no format specified, we might download all formats? Or safe default (JPG)?
                # Let's assume: if no format specified, download ALL available formats for that map/size.
                
                formats_to_process = [texture_format] if texture_format else list(formats.keys())

                for fmt in formats_to_process:
                    if fmt in formats:
                        file_info = formats[fmt]
                        # Some keys might be metadata only, ignore if no url
